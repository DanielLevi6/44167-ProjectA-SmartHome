...
# history = model.fit(datagen.flow(x_train,y_train,batch_size = 128), steps_per_epoch=STEPS, batch_size = 128, epochs=6, validation_data=(x_train, y_train),callbacks=[es])
#
...
history = model.fit(datagen.flow(x_train,y_train, batch_size = 128) ,epochs = 10 , validation_data = (x_train, y_train) , callbacks = [learning_rate_reduction])
